...
describe('AuditLogArchiveService', () => {
  ...
  beforeEach(() => {
    ...
    mockSequelize = {
      models: Object.fromEntries(
        allMainModels.map((m, i) => [`Model${i}`, m]),
      ),
    };
  ...
  describe('execute', () => {
    ...
    it('should process child tables for each batch', async () => {
      ...
      const entityModel = childMainModels.find((m) => m.tableName === 'audit_logs_entity');
      entityModel.findAll.mockResolvedValue([
        { id: 'entity-1', log_id: 'rec-1', entity: 'users' },
      ]);

      // No existing in archive
      const entityArchiveModel = childArchiveModels.find((m) => m.tableName === 'audit_logs_entity');
    ...
    it('should handle bulkCreate insert errors gracefully', async () => {
      const records = [
        { id: 'rec-1', createdAt: new Date('2025-01-01'), logType: 'REQUEST' },
      ];
    ...
    it('should handle child table processing errors gracefully', async () => {
      ...
      const errorModel = childMainModels.find((m) => m.tableName === 'audit_logs_error');
    ...
    it('should skip all child records that already exist in archive', async () => {
      ...
      const requestModel = childMainModels.find((m) => m.tableName === 'audit_logs_request');
      requestModel.findAll.mockResolvedValue([
        { id: 'req-1', log_id: 'rec-1' },
      ]);

      const requestArchiveModel = childArchiveModels.find((m) => m.tableName === 'audit_logs_request');
